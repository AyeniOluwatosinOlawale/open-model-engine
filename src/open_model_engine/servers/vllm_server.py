...
async def _get_vllm_metrics(base_url: str, timeout: float = 2.0) -> ServerMetrics:
    sm = ServerMetrics()
    try:
        async with httpx.AsyncClient(timeout=timeout) as c:
            ...
            for line in r.text.splitlines():
                ...
                try:
                    ...
                    continue
                if "vllm:cache_hit_rate" in key:
                    sm.kv_cache_hit_rate = val
                elif "vllm:gpu_cache_usage_perc" in key:
                    sm.kv_cache_utilization = val
                elif "vllm:num_requests_running" in key:
                    sm.num_running_requests = int(val)
                elif "vllm:num_requests_waiting" in key:
                    sm.num_queued_requests = int(val)
                elif "vllm:avg_prompt_throughput_toks_per_s" in key:
                    sm.prefill_throughput_tps = val
                elif "vllm:avg_generation_throughput_toks_per_s" in key:
                    sm.decode_throughput_tps = val
                # Histogram averages (vLLM 0.4.3+)
                elif "vllm:time_to_first_token_seconds_sum" in key:
                    sm.prefill_ms = val * 1000
                elif "vllm:time_per_output_token_seconds_sum" in key:
...
class VLLMServer(ServerBackend):
    backend = "vllm"
    ...
    async def get_server_metrics(self) -> ServerMetrics:
        return await _get_vllm_metrics(self.base_url)
